...
// after loading data via /restore_random. Reports p50, p90, p95, p99, etc.
//
// Usage:
//
//	go run ./cmd/latency              # run all endpoints
//	go run ./cmd/latency <name>       # run only endpoint matching <name>
package main
...
func runRestoreRandom(client *http.Client) error {
	...
	fmt.Printf("restore_random done (n=%d)\n", restoreN)
	return nil
}
...
func readPythonSection() string {
	b, err := os.ReadFile(reportHTMLPath)
	if err != nil {
		return `<p class="meta">Run: <code>cd python &amp;&amp; python test_apis_latency.py</code></p>`
	}
	s := string(b)
	i := strings.Index(s, markerPythonStart)
	j := strings.Index(s, markerPythonEnd)
	if i == -1 || j == -1 || j <= i {
		return `<p class="meta">Run: <code>cd python &amp;&amp; python test_apis_latency.py</code></p>`
	}
	body := strings.TrimSpace(s[i+len(markerPythonStart) : j])
	if body == "" {
		return `<p class="meta">Run: <code>cd python &amp;&amp; python test_apis_latency.py</code></p>`
	}
	return body
...
func readInmemSection() string {
	b, err := os.ReadFile(reportHTMLPath)
	if err != nil {
		return `<p class="meta">Run: <code>cd go &amp;&amp; go run ./cmd/latency_inmem</code></p>`
	}
	s := string(b)
	i := strings.Index(s, markerInmemStart)
	j := strings.Index(s, markerInmemEnd)
	if i == -1 || j == -1 || j <= i {
		return `<p class="meta">Run: <code>cd go &amp;&amp; go run ./cmd/latency_inmem</code></p>`
	}
	body := strings.TrimSpace(s[i+len(markerInmemStart) : j])
	if body == "" {
		return `<p class="meta">Run: <code>cd go &amp;&amp; go run ./cmd/latency_inmem</code></p>`
	}
	return body
...
func readDirectSection() string {
	b, err := os.ReadFile(reportHTMLPath)
	if err != nil {
		return `<p class="meta">Run: <code>cd go-fenwick-based-in-memory &amp;&amp; go run ./cmd/latency_direct</code></p>`
	}
	s := string(b)
	i := strings.Index(s, markerDirectStart)
	j := strings.Index(s, markerDirectEnd)
	if i == -1 || j == -1 || j <= i {
		return `<p class="meta">Run: <code>cd go-fenwick-based-in-memory &amp;&amp; go run ./cmd/latency_direct</code></p>`
	}
	body := strings.TrimSpace(s[i+len(markerDirectStart) : j])
	if body == "" {
		return `<p class="meta">Run: <code>cd go-fenwick-based-in-memory &amp;&amp; go run ./cmd/latency_direct</code></p>`
	}
	return body
...
func writeHTMLReport(results []resultRow) {
	...
	pythonSection := readPythonSection()
	inmemSection := readInmemSection()
	directSection := readDirectSection()
